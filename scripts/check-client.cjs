...
function dashboard(fetch) {
  ...
  vm.runInNewContext(ts.transpileModule(fs.readFileSync('src/app/page.tsx','utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, jsx: ts.JsxEmit.ReactJSX, esModuleInterop: true, target: ts.ScriptTarget.ES2022 } }).outputText, { exports, require: n => n==='react'?react:n==='@/components/InstallApp'||n==='@/components/ThemeToggle'||n==='@/components/ClosureAlerts'||n==='@/components/AnniversaryToast'?()=>null:n==='@/components/NumberTicker'?tickerExports:require(n), fetch, AbortController, AbortSignal, Date, console, document: { visibilityState: "visible", addEventListener(){}, removeEventListener(){} }, window: { addEventListener(){}, removeEventListener(){} }, setInterval: fn => { intervals.push(fn); return intervals.length; }, clearInterval(){} });
...
(async()=>{
  ...
  assert.doesNotMatch(page.render(), /Sources: A14 Eastbound|Sources: A14 Westbound/);
