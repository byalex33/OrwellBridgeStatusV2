...
const dependencies = {
  'next/server': { NextResponse: { json: body => body }, after: fn => fn() },
  '@/lib/traffic': { getBridgeTrafficData: async () => observation },
  '@/lib/cache': { cache: { getWithStale: key => ({ data: cache.get(key), isStale: false }), get: key => cache.get(key), set: (key, value) => cache.set(key, value) } },
